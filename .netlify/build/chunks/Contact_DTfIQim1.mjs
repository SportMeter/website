...
import 'kleur/colors';
import { b as $$Link } from './prerender_1IqFCGTC.mjs';
...
const $$Contact = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="items-center gap-x-[6.25rem] pb-40 auto-cols-fr justify-center flex" x-data="{
		isSubmitting: false,
		showSuccess: false,
		showError: false,
		errorMessage: '',
	...
	}"> <div class="col-span-1 row-span-1 w-full max-w-3xl"> <div class="items-center text-sm font-medium justify-center flex"> <h2 class="text-[3.13rem] leading-none font-semibold my-1">${introSection}</h2> </div> <h4 class=" mb-1 mt-10"> ${message} </h4> <div class="mt-8"> <!-- Success Message --> <div x-show="showSuccess" x-transition class="mb-6 p-4 bg-green-100 border border-green-400 text-green-700 rounded-md"> <p class="font-semibold">Message sent successfully!</p> <p>Thank you for contacting us. We'll get back to you soon.</p> </div> <!-- Error Message --> <div x-show="showError" x-transition class="mb-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-md"> <p class="font-semibold">Error:</p> <p x-text="errorMessage"></p> </div> <div class="mt-0 mx-0 mb-3.5"> <form @submit="submitForm" method="POST" action="/api/contact" class="items-center auto-cols-fr grid-cols-1 sm:grid-cols-2 grid-rows-[auto] grid gap-8"> ${fields?.map((field) => {
    if (field.type === "submit") {
      return renderTemplate`${renderComponent($$result, "Button", $$Button, { "type": "submit", "class:list": [field.width == 2 && "sm:col-span-2"], "x-bind:disabled": "isSubmitting" }, { "default": async ($$result2) => renderTemplate` <span x-show="!isSubmitting">${field.title}</span> <span x-show="isSubmitting" class="flex items-center"> <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"> <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle> <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg>
Sending...
</span> ` })}`;
